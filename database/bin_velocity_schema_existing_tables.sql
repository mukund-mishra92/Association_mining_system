-- Bin Velocity Analysis System - Updated Schema for Existing Tables
-- This schema works with existing sku_master and order tables

-- Note: This script assumes you have existing tables:
-- 1. sku_master (with velocity column that we can update)
-- 2. order (with order data for calculations)

-- We only need to create the additional tables for bin management

-- Table 1: Bin Configuration
...
-- Table 3: Velocity Calculation Parameters
...
-- Table 4: Velocity Calculation Jobs
...
-- Velocity Calculation History Table
-- Track when velocity was last updated for each SKU
CREATE TABLE IF NOT EXISTS sku_velocity_history (
    id INT AUTO_INCREMENT PRIMARY KEY,
    sku_code VARCHAR(100) NOT NULL,
    old_velocity INT,
    new_velocity INT NOT NULL CHECK (new_velocity IN (1, 2, 3)),
    order_frequency DECIMAL(10, 4) NOT NULL,
    ...
    INDEX idx_sku_code (sku_code),
    INDEX idx_calculation_date (calculation_date),
    INDEX idx_new_velocity (new_velocity)
);
...
-- View 1: Current SKU velocities from sku_master
...
SELECT 
    ...
    -- Add any other columns from sku_master that might be useful
    updated_at
FROM sku_master
WHERE sku_code IS NOT NULL;

-- View 2: Current bin scores with SKU details
...
SELECT 
    ...
    GROUP_CONCAT(sm.velocity ORDER BY bc.sku_code) as velocity_list,
    bvs.calculation_date
FROM bin_velocity_scores bvs
JOIN bin_configuration bc ON bvs.bin_id = bc.bin_id
LEFT JOIN sku_master sm ON bc.sku_code = sm.sku_code
...
-- View 3: SKU order summary for analysis
CREATE VIEW v_sku_order_summary AS
SELECT 
    o.sku_code,
    ...
    sm.velocity as current_velocity
FROM `order` o
LEFT JOIN sku_master sm ON o.sku_code = sm.sku_code
WHERE o.order_date >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)
GROUP BY o.sku_code, sm.velocity;

-- Stored procedure for velocity calculation status
DELIMITER //
CREATE PROCEDURE GetVelocitySystemStatus()
BEGIN
    SELECT 
        'SKU Master Records' as metric_type,
    ...
    SELECT 
        'Recent Orders (90 days)' as metric_type,
        COUNT(*) as total_count,
    ...
    SELECT 
        'Velocity Jobs' as metric_type,
        COUNT(*) as total_count,
        MAX(created_date) as last_update
    FROM velocity_calculation_jobs;
END//
DELIMITER ;

-- Stored procedure to get velocity distribution
DELIMITER //
CREATE PROCEDURE GetVelocityDistribution()
BEGIN
    SELECT 
        velocity as velocity_score,
        COUNT(*) as sku_count,
        ROUND((COUNT(*) * 100.0 / (SELECT COUNT(*) FROM sku_master WHERE velocity IS NOT NULL)), 2) as percentage
    FROM sku_master 
