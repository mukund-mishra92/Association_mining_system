...
Validate and return JSON:"""),
            ...
            try:
                ...
                has_doc_ref = "Document" in response or "📄" in response
                is_long_enough = len(response) > 50
                no_generic = not any(phrase in response.lower() for phrase in ["generally", "typically", "usually"])
                
                passed = has_doc_ref and is_long_enough and no_generic
                issues = []
                if not has_doc_ref:
                ...
                if not no_generic:
                    issues.append("Contains generic statements")
