...
def init_database(db_config: Optional[Dict[str, Any]]) -> DatabaseConnection:
    """Initialize database connection with optional custom configuration."""
    return DatabaseConnection(custom_config=db_config) if db_config else DatabaseConnection()


def connect_database(db: DatabaseConnection, task_id: str) -> bool:
    """Attempt DB connection with proper task error handling."""
    task_manager.update_progress(task_id, 0.1, "Connecting to database...")
    if not db.connect():
        task_manager.fail_task(task_id, "Failed to connect to database")
        return False
...
def retry_fetch_basket(db, days_back: Optional[int], task_id: str):
    """Retry fetch with relaxed conditions before failing."""
    fallback_days = max(days_back or 30, 90)

    task_manager.update_progress(
        task_id, 0.22, f"No data found. Retrying with lenient criteria ({fallback_days} days)..."
    )
    logger.warning(f"Retrying data fetch with fallback_days={fallback_days}, min_item_frequency=1")

    return db.fetch_order_data(
        days_back=fallback_days,
...
def normalize_scores(scores):
    """Normalize scores between 0.001 - 0.999."""
    ...
    return [
        0.001 + (score - min_s) / (max_s - min_s) * 0.998
        for score in scores
    ]


# =============================================================================
# ---------------------- Main Mining Background Task ---------------------------
# =============================================================================
...
def run_mining_task(
    ...
    """Main mining task executed asynchronously."""
    db = None
    try:
        task_manager.start_task(task_id, "Initializing mining process...")
        ...
        if not connect_database(db, task_id):
            return
        ...
        if df_basket is None or df_basket.empty:
            df_basket = retry_fetch_basket(db, days_back, task_id)
            if df_basket is None or df_basket.empty:
                task_manager.fail_task(
                    task_id,
                    "No data available even after retry.\nCheck tables + date ranges."
                )
                return

        # Prepare mining service
        mining_service = CleanAssociationMiningService(
            task_id=task_id,
            task_manager=task_manager,
            algorithm_params={
        ...
        # Run mining
        logger.info(f"Running mining pipeline for task {task_id}")
        recommendations = mining_service.run_mining_pipeline(df_basket)

        if recommendations.empty:
            task_manager.complete_task(
                task_id,
                result={"recommendations_count": 0, "rules": []},
        ...
        rec_sorted = recommendations.sort_values("composite_score", ascending=False)
        scores = normalize_scores(rec_sorted["composite_score"].astype(float).tolist())

        # Build UI rules
        ui_rules = []
        for idx, (_, rec) in enumerate(rec_sorted.head(100).iterrows()):
            ui_rules.append({
                "sku1": rec.get("main_item"),
                "sku2": rec.get("recommended_item"),
                "sku1_name": rec.get("main_item_name", ""),
                "sku2_name": rec.get("recommended_item_name", ""),
                "confidence": float(rec.get("confidence_score", 0)),
                "lift": float(rec.get("lift_score", 0)),
                "support": float(rec.get("support_score", 0)),
        ...
        # Save to DB
        task_manager.update_progress(task_id, 0.9, "Saving recommendations to database...")
        save_status = db.save_recommendations(rec_sorted)

        task_manager.complete_task(
            task_id,
            result={
                "recommendations_count": len(rec_sorted),
                "rules": ui_rules,
                "stats": {
                    "total_rules": len(rec_sorted),
                    "displayed_rules": len(ui_rules),
                    "database_saved": save_status,
                }
            },
        ...
        logger.exception("Error in mining task")
        task_manager.fail_task(task_id, f"Error in mining task: {str(e)}")
...
# ------------------------- FAST Mining Task ----------------------------------
# =============================================================================
# (Your FAST mining code stays same for now, but also cleaned if you want)
...
@router.post("/mine-rules", response_model=MiningStatusResponse)
async def mine_association_rules(request: MiningRequest, background_tasks: BackgroundTasks):
    """Starts enhanced/standard mining."""
    try:
        task_id = task_manager.create_task(
            task_type="association_mining",
        ...
        return MiningStatusResponse(
            status="started",
            task_id=task_id,
            message=f"Mining started ({'enhanced' if request.use_enhanced_mining else 'standard'})"
        )
    except Exception:
        logger.exception("Failed to start mining")
        raise HTTPException(500, "Failed to start mining process")


@router.get("/recommendations/{item_name}", response_model=ItemRecommendationsResponse)
async def get_item_recommendations(item_name: str, limit: int = 10):
    """Fetch recommendations for a specific SKU."""
    db = DatabaseConnection()
    try:
        if not db.connect():
            raise HTTPException(500, "Database connection failed")

        rec_list = db.get_recommendations(item_name, limit=limit)

        return ItemRecommendationsResponse(
            main_item=item_name,
            recommendations=[RecommendationResponse(**r) for r in rec_list]
        )
    except Exception:
...
async def get_all_tasks():
    tasks = task_manager.get_all_tasks()
    return {"count": len(tasks), "tasks": [t.to_dict() for t in tasks.values()]}


@router.get("/tasks/running")
async def get_running_tasks():
    tasks = task_manager.get_running_tasks()
    return {"count": len(tasks), "running_tasks": [t.to_dict() for t in tasks.values()]}
...
async def cleanup_old_tasks(max_age_hours: int = 24):
    task_manager.cleanup_old_tasks(max_age_hours)
    return {"message": f"Cleaned tasks older than {max_age_hours} hours"}
