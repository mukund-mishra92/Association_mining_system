...
class CleanAssociationMiningService:
    ...
    def __init__(self, task_id=None, task_manager=None, algorithm_params=None):
        ...
        # Use user-provided params or fall back to config defaults
    ...
    def _mine_rules_with_timeout(self, transactions, timeout_seconds):
        ...
        try:
            ...
            # Calculate adaptive support using user's min_support
            ...
            logger.info(f"User requested min_support={self.min_support:.3f}, using adaptive_support={adaptive_support:.3f}")
            ...
            if freq_itemsets is None or freq_itemsets.empty:
                ...
                logger.warning(f"Try lowering min_support (current user setting: {self.min_support}, adaptive: {adaptive_support:.3f})")
                return pd.DataFrame()
            
            logger.info(f"Found {len(freq_itemsets)} frequent itemsets")
            
            # Generate association rules using user's min_lift
            logger.info(f"Generating association rules with lift >= {self.min_lift}")
            ...
            if rules.empty:
                logger.warning(f"No rules found with lift >= {self.min_lift}")
                return pd.DataFrame()
            
            # Filter by confidence using user's min_confidence
    ...
    def _create_recommendations(self, rules):
        ...
        # Keep only top recommendations per item using user's max_recommendations
        rec_df = rec_df[rec_df['recommendation_rank'] <= self.max_recommendations]
        
        logger.info(f"Created {len(rec_df)} recommendations for {rec_df['main_item'].nunique()} items (top {self.max_recommendations} per item)")
        return rec_df