...
class DatabaseConnection:
    ...
    #         latest_date_query = f"""
    #         SELECT MAX(DATE(INSERTED_TIMESTAMP)) as max_date
    ...
    #             if days_back:
    #                 freq_query += f" AND DATE(o.INSERTED_TIMESTAMP) >= DATE_SUB('{max_date}', INTERVAL {days_back} DAY)"
    ...
    #         if days_back:
    #             query += f" AND DATE(o.INSERTED_TIMESTAMP) >= DATE_SUB('{max_date}', INTERVAL {days_back} DAY)"
...
#// ...existing code...
    ...
    def get_recommendations(self, item_name, limit=10):
        ...
        try:
            ...
            # Now get recommendations with JOINs to get SKU names
            query = f"""
            SELECT 
                child_sku.SKU_NAME as recommended_item_name,
                r.PROXIMITY_SCORE,
                r.SCORE_ID,
                r.CHILD_ARTICLE_ID as recommended_item_id
            FROM {self.recommendations_table} r
            INNER JOIN {self.sku_master_table} child_sku 
                ON r.CHILD_ARTICLE_ID = child_sku.SKU_ID
            WHERE r.PARENT_ARTICLE_ID = %s 
            ORDER BY r.PROXIMITY_SCORE DESC 
            LIMIT %s
            """
            
            self.cursor.execute(query, (parent_sku_id, limit))
            results = self.cursor.fetchall()
